...
import backup from "../asset/backup.jpeg";
...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=30bdeba8faf6e2ce0f0b8d6c3418b253`);
        ...
        <div className="max-w-2xl text-gray-600 text-lg dark:text-white">
          ...
          <p className="my-4">{movie.overview}</p>
            ...
            { movie.genres.map((genre) => (
              <span className="mr-2 border border-gray-600 rounded dark:border-gray-200 p-2" key={genre.id}>{genre.name}</span>
            )) }
            </p>
            ) : "" }
